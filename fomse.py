...
class Fomse(QMainWindow):
    def __init__(self):
        ...
        # Menu
        self.actionInfo.triggered.connect(self.open_info_window)  # Connect the About action to the info window
        ...
        # packBtn
        self.packBtn.clicked.connect(self.pack_files)  # Connect the pack button to the packing function

        # unpackBtn
        self.unpackBtn.clicked.connect(self.unpack_files)  # Connect the unpack button to the unpacking function

        # finBtn
        # probably unnecessary but eh
        self.finBtn.clicked.connect(self.close)  # Connect the finish button to the close function
        
        self.show()  # Show the main window

    def get_savefile(self):
        # Determine LocalAppData Path
        ...
        # append save dir
        saves_dir = os.path.join(local_app_data, 'FieldsofMistria\\saves')
    ...
    def get_editdir(self):
        # Open a file dialog to select a directory for editing
        local_app_data = os.getenv('LOCALAPPDATA')
        
        # append FoM dir
        fom_dir = os.path.join(local_app_data, 'FieldsofMistria')
    ...
    def get_exefile(self):
        # Open a file dialog to select the executable file
        local_app_data = os.getenv('LOCALAPPDATA')
    ...
    def pack_files(self):
        if self.curFile.text() == "":
            ...
            msgBox = QMessageBox()
            msgBox.setWindowTitle("No File Selected")
            msgBox.setText("Please select a save file to pack.")
            msgBox.exec()
            return
        if self.curDir.text() == "":
            # if the current directory line edit is empty, show a message box
            msgBox = QMessageBox()
            msgBox.setWindowTitle("No Directory Selected")
            msgBox.setText("Please select a directory to pack from.")
            msgBox.exec()
            return
        if self.curExec.text() == "":
            # if the current executable line edit is empty, show a message box
            msgBox = QMessageBox()
            msgBox.setWindowTitle("No Executable Selected")
            msgBox.setText("Please select the VaultC executable to pack the save file.")
            msgBox.exec()
            return
